...
class Checkbox(BaseConsoleRender):
    def __init__(self, *args, **kwargs):
        ...
        self.selection = [
            k
            for (k, v) in enumerate(self.question.choices)
            if v in self.default_choices()
        ]
        self.current = 0
    ...
    def get_options(self):
        ...
        for index, choice in enumerate(cchoices):
            if (
                ...
                or (
                    is_in_end
                    and index + max(len(choices) - MAX_OPTIONS_DISPLAYED_AT_ONCE, 0)
                    in self.selection
                )
            ):  # noqa
    ...
    def process_input(self, pressed):
        ...
        if pressed == key.UP:
            ...
            self.selection = [
                i for i in range(len(self.question.choices)) if i not in self.selection
            ]
