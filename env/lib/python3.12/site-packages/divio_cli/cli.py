...
def application_list(obj, grouped, pager, as_json):
    ...
    for application in api_response["results"]:
        org_name = obj.client.get_organisation(application["organisation"])["name"]
        ...
        for organisation in data:
            output_items.append(
                "{title}\n{line}\n\n{table}\n\n".format(
                    ...
                    table=table(sort_applications(data[organisation]), header[:3]),
                )
            )
...
def application_deploy(obj, remote_id, environment, build_mode):
    """Deploy application."""
    obj.client.deploy_application_or_get_progress(remote_id, environment, build_mode)
...
def application_dashboard(obj, remote_id):
    ...
    launch_url(
        get_cp_url(client=obj.client, application_id=remote_id, zone=obj.zone or zone)
    )
...
@click.pass_obj
@allow_remote_id_override
def list_deployments(obj, remote_id, environment, all_environments, limit_results):
    """
    ...
    if obj.as_json:
        ...
        for result in results:
            content_table_title = (
                f"Environment: {result['environment']} ({result['environment_uuid']})"
            )
            columns = obj.table_format_columns
            rows = [[row[key] for key in columns] for row in result["deployments"]]
...
def get_deployment(obj, remote_id, deployment_uuid):
    ...
    if obj.as_json:
        ...
        content_table_title = (
            f"Environment: {response['environment']} ({response['environment_uuid']})"
        )
        ...
        content_table = table(rows, headers=(), tablefmt="grid", maxcolwidths=50)
        content = f"{content_table_title}\n{content_table}"
...
@click.pass_obj
@allow_remote_id_override
def get_deployment_environment_variable(obj, remote_id, deployment_uuid, variable_name):
    """
...
def list_environment_variables(
    ...
    if obj.as_json:
        ...
        echo_environment_variables_as_txt(results, obj, all_environments, environment)
    else:
        content = ""
        for result in results:
            content_table_title = (
            ...
            content_table = table(rows, columns, tablefmt="grid", maxcolwidths=50)
            content += f"{content_table_title}\n{content_table}\n\n"
    ...
    if results:
        if obj.as_json:
            ...
            for result in results:
                ...
                content_table = table(row, columns, tablefmt="grid", maxcolwidths=50)
                content += f"{content_table_title}\n{content_table}\n\n"
            echo_large_content(content.strip("\n"), ctx=obj)
    else:
        click.secho(
...
def application_setup(obj, slug, environment, path, overwrite, skip_doctor):
    ...
    localdev.create_workspace(obj.client, slug, environment, path, overwrite, obj.zone)
...
@click.pass_obj
@allow_remote_id_override
def list_service_instances(obj, remote_id, environment, as_json, limit_results):
    """List the services instances of an application."""

    try:
        environment_uuid = obj.client.get_environment(remote_id, environment)["uuid"]
    except KeyError:
        click.secho(
    ...
    if not results:
        click.echo(f"No service instances found for {environment!r} environment.")
        return
...
@click.pass_obj
@allow_remote_id_override
def add_service_instances(obj, remote_id, environment, prefix, region, service):
...
@common_pull_options
def pull_db(obj, remote_id, environment, prefix, keep_tempfile, backup_si_uuid):
    """
...
@common_pull_options
def pull_media(obj, remote_id, environment, prefix, keep_tempfile, backup_si_uuid):
    """
