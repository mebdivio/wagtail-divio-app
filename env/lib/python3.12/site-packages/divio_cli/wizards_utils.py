...
APP_WIZARD_MESSAGES = {
    ...
    "org_missing": (
        "Missing option '-o' / '--organisation'. " "Required in non-interactive mode."
    ),
    "org_invalid": "Invalid organization.",
    # Plan group
    "plan_group_select": "Select a plan for your application",
    "plan_group_missing": (
        "Missing option '-p' / '--plan-group'. " "Required in non-interactive mode."
    ),
...
def verify_app_repo(client, uuid, branch):
    ...
    if repo_state != "CLONED":
        if repo_state == "CLONING":
            status_print(APP_WIZARD_MESSAGES["repo_verification_timeout"], "error")
        else:
            status_print(APP_WIZARD_MESSAGES["repo_verification_failed"], "error")
        verification_status = inquirer.prompt(options, raise_keyboard_interrupt=True)[
            "choice"
        ]
    else:
