...
def get_divio_zone():
    try:
        application_specific_zone = get_project_settings(get_application_home()).get(
            "zone", None
        )
...
class CloudClient:
    ...
    def get_services(self, region_uuid=None, application_uuid=None, limit_results=None):
        kwargs = {}

        # TODO this smells like a security issue
        kwargs["filter_region"] = f"region={region_uuid}" if region_uuid else ""
    ...
    def ssh(self, application_uuid, environment):
        ...
        try:
            env = self.get_environment_by_application(application_uuid, environment)
    ...
    def get_environments(self, params=None):
        if params is None:
            params = {}
        try:
            return api_requests.EnvironmentsListRequest(self.session, params=params)()
        except (KeyError, json.decoder.JSONDecodeError):
            raise DivioException("Error establishing connection.")
        ...
        if environment:
            ...
            if last_deployment_uuid:
                try:
                    ...
                    return (
                        f"Deployment ID: {deploy_log['uuid']}\n\n{deploy_log['logs']}"
                    )
    ...
    def deploy_application_or_get_progress(
        ...
        try:
            response = self.get_deployment_by_application(application_uuid, env["uuid"])
    ...
    def get_deployment_by_application(self, application_uuid, environment_uuid):
    ...
    def create_backup_download(self, backup_uuid, backup_service_instance_uuid):
        ...
        try:
            ...
            raise DivioException("Error while fetching service instance backups.")
    ...
    def backup_upload_request(
        ...
        return api_requests.CreateBackupUploadRequest(self.session, data=data)()
    ...
    def create_backup_restore(
        ...
        return api_requests.CreateBackupRestoreRequest(self.session, data=data)()
