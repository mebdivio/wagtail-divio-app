...
def test_create_backup():
    ...
    ret = backups.create_backup(client, "<website_id>", "environment", backups.Type.DB)
...
def test__upload_backup_exoscale(monkeypatch, file_size, max_size, ok):
    ...
    if ok:
        ...
        mock_file.assert_called_with("file", "rb")
        requests.put.assert_called_once_with(upload_params["url"], data=mock_file())
    else:
        with pytest.raises(
            DivioException,
...
def test__wait_for_backup_to_complete(statuses, ok):
    ...
    if ok:
        ...
        assert f"Backup failed: success={statuses[-1][-1]}" in str(excinfo.value)
...
def test__wait_for_backup_to_complete_si_error(si_backups, si_details, message):
    client = MagicMock()
    ...
    with patch("time.sleep"):
        with pytest.raises(DivioException) as excinfo:
            backups._wait_for_backup_to_complete(client, "<uuid>", message="message")
