...
class CreateAppWizard:
    ...
    def get_org(self, org: str) -> tuple[str, str]:
        ...
        orgs_uuid_name_mapping = {org["uuid"]: org["name"] for org in user_orgs}
        ...
        if not self.interactive:
            ...
            while True:
                if not org:
                    ...
                    org = inquirer.prompt(options, raise_keyboard_interrupt=True)[
                        "uuid"
                    ]
    ...
    def get_region(self, region: str, plan_group: str) -> tuple[str, str]:
        ...
        user_regions_uuids = self.client.get_application_plan_group(plan_group)[
            "regions"
        ]
        user_regions, _ = self.client.get_regions(params={"uuid": user_regions_uuids})
        ...
        if not self.interactive:
            ...
            while True:
                if not region:
                    ...
                    region = inquirer.prompt(options, raise_keyboard_interrupt=True)[
                        "uuid"
                    ]
                if region not in user_regions_uuids:
                    status_print(
    ...
    def get_template(self, template: str) -> tuple[str | None, str | None]:
        ...
        # Interactive mode
            ...
            create_template = (
                "custom"
                if template
                else inquirer.prompt(options, raise_keyboard_interrupt=True)["choice"]
            )
            ...
            # Custom template
                ...
                while True:
                    if not template:
                        template = prompt(APP_WIZARD_MESSAGES["template_enter_url"])
    ...
    def get_template_release_commands(
        ...
        return self.client.get_application_template(template_uuid)["release_commands"]

    def get_services(self, template_uuid: str | None, region: str) -> list[dict] | None:
        """
        ...
        template_services_config = self.client.get_application_template(template_uuid)[
            "services"
        ]
    ...
    def get_release_commands(self, template_uuid: str | None) -> list[dict] | None:
        """
        ...
        template_release_commands = self.get_template_release_commands(template_uuid)
    ...
    def get_git_repo(self, org: str) -> tuple[str | None, str | None, str | None]:
        """
        ...
        while True:
            if not (restart_connection or confirm(APP_WIZARD_MESSAGES["repo_connect"])):
                return None, None, None
            else:
                # URL
                ...
                if auth_type == "ssh":
                    ...
                    ssh_key_type_options = [
                        inquirer.List(
                            "key",
                            message=APP_WIZARD_MESSAGES["repo_ssh_key_type_select"],
                    ...
                    if confirm(APP_WIZARD_MESSAGES["create_deploy_key"], default=True):
                        while True:
    ...
    def create_app(
        ...
        if not self.interactive:
            ...
            if self.verbose:
                ...
                echo("\nApplication Details")
                echo(json.dumps(app_details, indent=4) if self.as_json else app_details)
                echo()
