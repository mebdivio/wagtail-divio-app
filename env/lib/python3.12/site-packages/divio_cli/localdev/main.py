...
def get_git_host(zone=None):
    try:
        git_host = get_project_settings(get_application_home()).get("git_host", None)
...
def get_git_clone_url(slug, application_uuid, client, zone=None):
    ...
    return GIT_CLONE_URL.format(git_host=get_git_host(zone=zone), project_slug=slug)
...
def configure_project(website_slug, path, client, zone=None):
    ...
    click.secho("Configuration file: {}".format(click.style(path, fg="bright_green")))
    click.secho(
        "Git remote:         {}".format(
            click.style(
                get_git_clone_url(website_slug, application_uuid, client, zone=zone),
                fg="bright_green",
            )
        )
    )
...
def create_workspace(
    ...
    configure_project(website_slug=website_slug, path=path, client=client, zone=zone)
...
def pull_media(
    ...
    application_uuid = utils.get_project_settings(project_home)["application_uuid"]
...
def push_media(client, environment, application_uuid, prefix, keep_tempfile=True):
...
def update_local_application(git_branch, client, strict=False):
    ...
    remote_dsn = client.get_repository_dsn(
        utils.get_project_settings(utils.get_application_home())["application_uuid"]
    )
