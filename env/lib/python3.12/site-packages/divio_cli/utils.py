...
def status_print(message, status="default", **kwargs):
    ...
    if status in status_colors:
        ...
        raise ValueError(
            f"Unknown status {status!r}. " f"Choices are: {', '.join(status_colors)}."
        )
...
def open_application_cloud_site(client, application_id, environment):
    try:
        env = client.get_environment_by_application(application_id, environment)
...
def json_response_request_paginate(
    ...
    try:
        ...
        while True:
            ...
            if limit_results and (not next_page or len(results) >= limit_results):
...
def echo_environment_variables_as_txt(
    ...
    for result in results:
        ...
        if result_content:
            ...
            result_content.insert(0, f"{result_title}\n{'-'*len(result_title)}\n")
