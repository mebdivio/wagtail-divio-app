...
class Table:
    ...
    def _write_row(self, buffer, column_widths, row):
        ...
        for column_index, cell in enumerate(row):
            cell_lines.append(wrap(str(cell).strip(), column_widths[column_index]))
        ...
        for row in self._rows:
            for column_index, cell in enumerate(row):
                ...
                longest_cell_line = max(
                    [0] + [get_print_length(cell_line) for cell_line in cell_lines],
                )
...
def select_option(
    ...
    def print_help():
        ...
        for index, label in enumerate(labels):
            line = f" {label.rjust(longest_label_length)} - {help_texts[index]}"
    ...
    if not choices:
        ...
        choices_indexes = [
            get_choice_index(choice) for choice in choices if choice is not None
        ]
