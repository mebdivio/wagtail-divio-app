...
class _LibHeifImageFile(ImageFile.ImageFile):
    ...
    def load(self):
        if self._heif_file:
            ...
            if (
                self.fp
                and getattr(self, "_exclusive_fp", False)
                and hasattr(self.fp, "close")
            ):
                self.fp.close()
            self.fp = None
    ...
    def seek(self, frame: int):
        ...
        if pil_version[:3] != "10.":
            ...
            self.im = Image.core.new(
                self._mode, self._size
            )  # pylint: disable=too-many-function-args

        _exif = getattr(
            self, "_exif", None
        )  # Pillow 9.2+ do no reload exif between frames.
...
def register_avif_opener(**kwargs) -> None:
    ...
    if not _pillow_heif.get_lib_info()["AVIF"]:
        warn(
            "This version of `pillow-heif` was built without AVIF support.",
            stacklevel=1,
        )
        return
    __options_update(**kwargs)
...
def __save_one(
    im: Image.Image, fp: IO[bytes], compression_format: HeifCompressionFormat
):
...
def __save_all(
    ...
    primary_index = _get_primary_index(
        chain(ImageSequence.Iterator(im), append_images),
        im.encoderinfo.get("primary_index", None),
    )
...
def _pil_encode_image(
    ctx: CtxEncode, img: Image.Image, primary: bool, **kwargs
) -> None:
    ...
    if img.mode == "YCbCr":
        ctx.add_image_ycbcr(
            img, image_orientation=_get_orientation_for_encoder(_info), **_info
        )
    else:
