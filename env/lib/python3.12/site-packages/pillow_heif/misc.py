...
def _get_orientation_xmp(
    info: dict, exif_orientation: int | None, reset: bool = False
) -> int | None:
    xmp_orientation = 1
    if info.get("xmp"):
        ...
        if xmp_data[0]:
            ...
            if decoded_xmp_data:
                ...
                if match:
                    ...
                    if reset:
                        decoded_xmp_data = re.sub(
                            r'tiff:Orientation="([0-9])"', "", decoded_xmp_data
                        )
                        decoded_xmp_data = re.sub(
                ...
                if encoding != "utf-8" or xmp_orientation != 1:
                    info["xmp"] = b"".join(
                        [
                            decoded_xmp_data.encode("utf-8"),
                            b"\x00" if len(xmp_data) > 1 else b"",
                        ]
                    )
    return (
        xmp_orientation if exif_orientation is None and xmp_orientation != 1 else None
    )
...
def _retrieve_exif(metadata: list[dict]) -> bytes | None:
    ...
    for i, md_block in enumerate(metadata):
        if md_block["type"] == "Exif":
            _purge.append(i)
            skip_size = int.from_bytes(
                md_block["data"][:4], byteorder="big", signed=False
            )
            skip_size += 4  # skip 4 bytes with offset
            if (
                len(md_block["data"]) - skip_size <= 4
            ):  # bad EXIF data, skip first 4 bytes
                skip_size = 4
            elif (
                skip_size >= 6
                and md_block["data"][skip_size - 6 : skip_size] == b"Exif\x00\x00"
            ):
                skip_size -= 6
...
def _get_heif_meta(c_image) -> dict:
    r = {}
    _camera_intrinsic_matrix = __get_camera_intrinsic_matrix(
        c_image.camera_intrinsic_matrix
    )
...
class CtxEncode:
    ...
    def __init__(self, compression_format: HeifCompressionFormat, **kwargs):
        ...
        self.ctx_write = _pillow_heif.CtxWrite(
            ...
            options.PREFERRED_ENCODER.get(
                "HEIF" if compression_format == HeifCompressionFormat.HEVC else "AVIF",
                "",
            ),
        )
    ...
    def add_image(self, size: tuple[int, int], mode: str, data, **kwargs) -> None:
        ...
        # creating image
        im_out = self.ctx_write.create_image(
            size, MODE_INFO[mode][2], MODE_INFO[mode][3], premultiplied_alpha
        )
        # image data
        if MODE_INFO[mode][0] == 1:
            im_out.add_plane_l(
                ...
                data,
                kwargs.get("stride", 0),
                HeifChannel.CHANNEL_Y,
            )
        elif MODE_INFO[mode][0] == 2:
            im_out.add_plane_la(
                size, bit_depth_out, bit_depth_in, data, kwargs.get("stride", 0)
            )
        else:
            im_out.add_plane(
                size,
    ...
    def add_image_ycbcr(self, img: Image.Image, **kwargs) -> None:
        ...
        # creating image
        im_out = self.ctx_write.create_image(
            img.size, MODE_INFO[img.mode][2], MODE_INFO[img.mode][3], 0
        )
        # image data
        for i in (
            HeifChannel.CHANNEL_Y,
            HeifChannel.CHANNEL_CB,
            HeifChannel.CHANNEL_CR,
        ):
            im_out.add_plane_l(
                img.size, 8, 8, bytes(img.getdata(i)), kwargs.get("stride", 0), i
            )
    ...
    def _finish_add_image(self, im_out, size: tuple[int, int], **kwargs):
        ...
        if __icc_profile is not None:
            im_out.set_icc_profile(
                kwargs.get("icc_profile_type", "prof"), __icc_profile
            )
    ...
    def save(self, fp) -> None:
        ...
        if isinstance(fp, (str, Path)):
            ...
            raise TypeError(
                "`fp` must be a path to file or an object with `write` method."
            )
