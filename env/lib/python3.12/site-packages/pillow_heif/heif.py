...
class HeifImage(BaseImage):
    ...
    def __init__(self, c_image):
        ...
        _thumbnails: list[int | None] = (
            [i for i in c_image.thumbnails if i is not None]
            if options.THUMBNAILS
            else []
        )
        _depth_images: list[HeifDepthImage | None] = (
            [HeifDepthImage(i) for i in c_image.depth_image_list if i is not None]
    ...
    def __repr__(self):
        _bytes = (
            ...
            if self._data or isinstance(self._c_image, MimCImage)
            else "no"
        )
        return (
...
def _encode_images(images: list[HeifImage], fp, **kwargs) -> None:
    ...
    compression_format = (
        ...
        if compression == "AVIF"
        else HeifCompressionFormat.HEVC
    )
