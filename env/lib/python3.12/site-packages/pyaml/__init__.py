...
class PYAMLDumper(yaml.dumper.SafeDumper):
    ...
    def __init__(
        ...
        **kws,
        ...
        if isinstance(sort_dicts, PYAMLSort):
            if sort_dicts is sort_dicts.none:
                kws["sort_keys"] = False
            elif sort_dicts is sort_dicts.keys:
                kws["sort_keys"] = True
            else:
                self.pyaml_sort_dicts, kws["sort_keys"] = sort_dicts, False
        elif sort_dicts is not None:
            kws["sort_keys"] = sort_dicts  # for compatibility
    ...
    def pyaml_transliterate(s):
        if unidecode_missing := not all(ord(c) < 128 for c in s):
            ...
            if unidecode:
                unidecode_missing, s = None, unidecode(s)
    ...
    def anchor_node(self, node, hints=list()):
        if node in self.anchors:
            if self.anchors[node] is None and not self.pyaml_force_embed:
                if hints:
                    nid, uc = self.pyaml_transliterate(
                        "_-_".join(h.value for h in hints)
                    )
                    if len(nid) > (n := self.pyaml_anchor_len_max - 9) + 9:
                        nid = (
                            f"{nid[:n//2]}-_-{nid[-n//2:]}_{self.generate_anchor(node)}"
                        )
                    elif uc is True:
                        nid = f"{nid}_{self.generate_anchor(node)}"
                else:
                    nid = self.generate_anchor(node)
    ...
    def represent_mapping_sort_oneline(self, kv):
        ...
        if isinstance(key, (int, float)):
            ...
            k, key = 3, f"{type(key)}\0{key}"  # best-effort sort for all other types
        return v, k, key
    ...
    def represent_mapping(self, tag, mapping, *args, **kws):
        if self.pyaml_sort_dicts is PYAMLSort.oneline_group:
            try:
                mapping = dict(
                    sorted(mapping.items(), key=self.represent_mapping_sort_oneline)
                )
            except TypeError:
                pass  # for subtype comparison fails
        return super().represent_mapping(tag, mapping, *args, **kws)

    def represent_undefined(self, data):
        if (
            isinstance(data, tuple)
            and hasattr(data, "_make")
            and hasattr(data, "_asdict")
        ):
            return self.represent_dict(data._asdict())  # assuming namedtuple
        if isinstance(data, cs.abc.Mapping):
            return self.represent_dict(data)  # dict-like
        ...
        if hasattr(type(data), "__dataclass_fields__"):
            try:
                ...
                pass  # can still be something else
            else:
                return self.represent_dict(dcs.asdict(data))
        try:  # this is for numpy arrays, and the likes
            if not callable(getattr(data, "tolist", None)):
                raise AttributeError
        except:
            pass  # can raise other errors with custom types
        else:
            return self.represent_data(data.tolist())
        if self.pyaml_repr_unknown:  # repr value as a short oneliner
            if isinstance(n := self.pyaml_repr_unknown, bool):
                n = 50
            ...
            if (st := f"{cls.__module__}.{cls.__name__}") in s:
                st = "value"
            node.value = (s := self.str_ext(s))
            s.ext = f"# python {st}"
            return node
    ...
    def write_ext(self, func, text, *args, **kws):
        ...
        if ext := getattr(text, "ext", None):
            super().write_plain(ext, split=False)

    write_folded = lambda s, v, *a, **kw: s.write_ext("folded", v, *a, **kw)
    write_literal = lambda s, v, *a, **kw: s.write_ext("literal", v, *a, **kw)
    write_single_quoted = lambda s, v, *a, **kw: s.write_ext(
        "single_quoted", v, *a, **kw
    )
    write_double_quoted = lambda s, v, *a, **kw: s.write_ext(
        "double_quoted", v, *a, **kw
    )
...
add_representer(
    bool, lambda s, o: s.represent_scalar("tag:yaml.org,2002:bool", ["no", "yes"][o])
)
add_representer(
...
def dump_add_vspacing(
    yaml_str, split_lines=40, split_count=2, oneline_group=False, oneline_split=False
):
    ...
    def _add_vspacing(lines):
        ...
        for n, line in enumerate(lines):
            ...
            if ind_re_sub:
                if ind_re_sub.match(line):
                    has_sub = True
                    continue
                if n - a > split_lines and (block := lines[a:n]):
                    if a_seq:
                        block.insert(0, lines[a - 1].replace("- ", "  ", 1))
                    blocks.append((a, n, _add_vspacing(block)[a_seq:]))
                ind_re_sub = None
            if ind_re.match(line):
                item_lines.append(n)
            if m := re.match(r"( *)(- )?\S.*:(\s|$)", line):
                a, a_seq, ind_re_sub = n + 1, bool(m[2]), re.compile(m[1] + " ")
        if (
            split_items := len(lines) > split_lines
            and len(item_lines) > split_count
            and (oneline_split or has_sub)
        ):
            for n in item_lines:
                try:
                    if (
                    ...
                    continue
                lines[n] = f"\n{lines[n]}"
        for a, b, block in reversed(blocks):
            lines[a:b] = block
        if ind_re:
            lines.pop()
        if split_items:
...
def dump(
    ...
    **pyyaml_kws,
    ...
    if safe is not None:
        ...
        warnings.warn(
            'pyaml module "safe" arg/keyword is ignored as implicit'
            " safe=maybe-true?, as of pyaml >= 23.x",
            category=cat,
            stacklevel=2,
        )
    if sort_dicts is not None and not isinstance(sort_dicts, PYAMLSort):
        warnings.warn(
            "Using pyaml module sort_dicts as boolean is deprecated as of"
    ...
    buff = io.StringIO()
    Dumper = lambda *a, **kw: PYAMLDumper(
        *a,
        **kw,
        force_embed=force_embed,
        string_val_style=string_val_style,
        sort_dicts=sort_dicts,
        repr_unknown=repr_unknown,
    )
    if not multiple_docs:
        data = [data]
    else:
        pyyaml_kws.setdefault("explicit_start", True)
    yaml.dump_all(
        data,
        buff,
        Dumper=Dumper,
    ...
    if vspacing not in [None, False]:
        if vspacing is True:
            ...
            warnings.warn(
                ...
                " enabling default vspacing behavior.",
                DeprecationWarning,
                stacklevel=2,
            )
            vspacing = dict()
        if sort_dicts is PYAMLSort.oneline_group:
            vspacing.setdefault("oneline_group", True)
    ...
    if dst is bytes:
        ...
        try:
            dst.write(b"")  # tests if dst is str- or bytestream
        except:
            dst.write(buff)
        else:
            dst.write(buff.encode())
