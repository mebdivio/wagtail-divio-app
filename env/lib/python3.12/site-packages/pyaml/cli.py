...
def safe_replacement(path, *open_args, mode=None, xattrs=None, **open_kws):
    ...
    if xattrs is None and getattr(os, "getxattr", None):  # MacOS
        try:
            xattrs = dict((k, os.getxattr(path, k)) for k in os.listxattr(path))
        except FileNotFoundError:
            pass
            ...
            if err.errno != errno.ENOTSUP:
                raise
    open_kws.update(
        delete=False, dir=os.path.dirname(path), prefix=os.path.basename(path) + "."
    )
    if not open_args:
        open_kws.setdefault("mode", "w")
...
def main(argv=None, stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr):
    ...
    dd = lambda text: re.sub(
        r" \t+", " ", textwrap.dedent(text).strip("\n") + "\n"
    ).replace("\t", "  ")
    ...
    if vspacing := opts.vspacing:
        ...
        if flags:
            if "s" in flags:
                vspacing["oneline_split"] = True
            if "g" in flags:
                pyaml_kwargs["sort_dicts"] = pyaml.PYAMLSort.oneline_group
        if vsplit:
            lines, _, count = vsplit.strip().strip("/").partition("/")
            if lines:
                vspacing["split_lines"] = int(lines.strip())
            if count:
                vspacing["split_count"] = int(count.strip())
        if vspacing:
            pyaml_kwargs["vspacing"] = vspacing

    if len(data) > 1:
        ys = pyaml.dump_all(data, **pyaml_kwargs)
    else:
        ys = pyaml.dump(data[0], **pyaml_kwargs)  # avoids leading "---"

    if not opts.quiet:
        try:
            data_chk = list(yaml.safe_load_all(ys))
            try:
                data_hash = json.dumps(data, sort_keys=True)
            except:
                pass  # too complex for checking with json
            else:
            ...
            p_err(
                "WARNING: Failed to parse produced YAML"
