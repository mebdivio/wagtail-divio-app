...
data = dict(
    key="value",
    path="/some/path",
    query_dump=dict(key1="тест1", key2="тест2", key3="тест3", последний=None),
    ids=dict(),
    a=[1, None, "asd", "не-ascii"],
    b=3.5,
    c=None,
)
...
class CliToolTests(unittest.TestCase):
    ...
    def test_success(self):
        ...
        ys = yaml.safe_dump(d)
        pyaml.cli.main(argv=list(), stdin=io.StringIO(ys), stdout=out, stderr=err)
        yaml.safe_load(out.getvalue())
        self.assertGreater(len(out.getvalue()), 150)
        self.assertEqual(err.getvalue(), "")

        d.update(d=test_const.heartbeat, asd=cs.OrderedDict(b=1, a=2))
    ...
    def test_out_err_nocheck(self):
        ...
        try:
            ys = yaml.safe_dump(d)
            pyaml.cli.main(argv=["-q"], stdin=io.StringIO(ys), stdout=out, stderr=err)
            with self.assertRaises(yaml.YAMLError):
                yaml.safe_load(out.getvalue())
            self.assertGreater(len(out.getvalue()), 150)
            self.assertEqual(err.getvalue(), "")
    ...
    def test_replace(self):
        ...
        with self.assertRaises(SystemExit):
            ...
            try:
                pyaml.cli.main(argv=["-r"], stdin=io.StringIO(), stdout=out, stderr=err)
            finally:
        ...
        with tempfile.NamedTemporaryFile(prefix=".pyaml.test.") as tmp:
            ...
            pyaml.cli.main(
                argv=["-r", tmp.name], stdin=io.StringIO(), stdout=out, stderr=err
            )
    ...
    def test_lines(self):
        ...
        ys += "\n" + json.dumps(d3)
        ys += "\0" + json.dumps(d3)
        pyaml.cli.main(argv=["-l"], stdin=io.StringIO(ys), stdout=out, stderr=err)
        with self.assertRaises(yaml.YAMLError):
            pyaml.cli.main(argv=list(), stdin=io.StringIO(ys), stdout=out, stderr=err)
        self.assertGreater(len(out.getvalue()), 150)
        self.assertEqual(err.getvalue(), "")
        ...
        out, ys = io.StringIO(), "\n\0" + json.dumps(d3) + "\n \n\0\t\0\n"
        pyaml.cli.main(argv=["--lines"], stdin=io.StringIO(ys), stdout=out, stderr=err)
